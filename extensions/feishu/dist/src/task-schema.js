...
export const FeishuTaskSchema = Type.Object({
  ...
  due: Type.Optional(
    Type.String({
      description:
        "[create/update] 截止时间。格式: ISO 8601 (2025-03-01T10:00:00+08:00) 或 Unix 秒级时间戳",
    }),
  ),
  ...
  // --- add_reminder ---
  relative_fire_minutes: Type.Optional(
    Type.Number({
      description:
        "[add_reminder] 提醒时间（相对于截止时间的分钟数，负数表示提前）。如 -30 表示截止前30分钟提醒",
    }),
  ),
  // --- list ---
  page_size: Type.Optional(
    Type.Number({ description: "[list] 每页数量 (1-100, 默认 50)", minimum: 1, maximum: 100 }),
  ),
  ...
  sender_open_id: Type.Optional(
    Type.String({
      description:
        "[create] The open_id (ou_xxx) of the message sender. When provided, the sender will automatically be added as an assignee of the new task.",
    }),
  ),
  // --- add_comment ---
