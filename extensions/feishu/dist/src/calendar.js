...
export async function updateCalendarEvent(client, calendarId, eventId, updates) {
  ...
  if (updates.startTime) data.start_time = { timestamp: toTimestamp(updates.startTime) };
  if (updates.endTime) data.end_time = { timestamp: toTimestamp(updates.endTime) };
  if (updates.location) data.location = { name: updates.location };
...
export async function addEventAttendees(client, calendarId, eventId, attendees) {
  const attendeeList = attendees.map((a) => {
    ...
    if (type === "user") return { type: "user", user_id: a.id };
    if (type === "chat") return { type: "chat", chat_id: a.id };
...
export function registerFeishuCalendarTools(api) {
  ...
  const getClient = () => {
    if (!api.config) throw new Error("No configuration available");
    const accounts = listEnabledFeishuAccounts(api.config);
    if (accounts.length === 0) throw new Error("No Feishu accounts configured");
    const account = accounts[0];
    const toolsCfg = resolveToolsConfig(account.config.tools);
    if (!toolsCfg.calendar) throw new Error("Calendar tools are disabled in configuration");
      ...
      async execute(_toolCallId, params) {
        ...
        try {
          ...
          switch (action) {
            ...
            case "update_event":
              return json(
                await updateCalendarEvent(client, p.calendar_id, p.event_id, {
                  summary: p.summary,
