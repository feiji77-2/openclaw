  ...
  api.registerTool(
    {
      ...
      description:
        "Search/filter records in a Bitable table with conditions. Supports AND/OR filters, sorting, and field selection.",
  ...
  api.registerTool(
    {
      ...
      description:
        "Create a new field (column) in a Bitable table. Common types: 1=Text, 2=Number, 3=SingleSelect, 4=MultiSelect, 5=DateTime, 7=Checkbox, 11=User, 15=URL",
  ...
  api.registerTool(
    {
      ...
      description:
        "Create a new view for a Bitable table. View types: grid(table), kanban, gallery, form, gantt",
...
const SearchRecordsSchema = Type.Object({
  ...
  filter: Type.Optional(
    Type.Any({
      description:
        'Filter conditions. Example: {"conjunction":"and","conditions":[{"field_name":"Status","operator":"is","value":["Done"]}]}',
    }),
  ),
  sort: Type.Optional(
    Type.Any({
  ...
  field_names: Type.Optional(
    Type.Array(Type.String(), { description: "Specific field names to return (reduces data)" }),
  ),
  page_size: Type.Optional(
    Type.Number({ description: "Records per page (1-500, default 100)", minimum: 1, maximum: 500 }),
  ),
...
const ListTablesSchema = Type.Object({
  app_token: Type.String({ description: "Bitable app token" }),
  page_size: Type.Optional(
    Type.Number({ description: "Tables per page (default 20)", minimum: 1, maximum: 100 }),
  ),
  page_token: Type.Optional(Type.String({ description: "Pagination token" })),
});
...
const CreateFieldSchema = Type.Object({
  ...
  type: Type.Number({
    description:
      "Field type: 1=Text, 2=Number, 3=SingleSelect, 4=MultiSelect, 5=DateTime, 7=Checkbox, 11=User, 15=URL",
  }),
...
const CreateViewSchema = Type.Object({
  ...
  view_type: Type.Optional(
    Type.String({ description: "View type: grid, kanban, gallery, form, gantt. Default: grid" }),
  ),
});
const ListDashboardsSchema = Type.Object({
  app_token: Type.String({ description: "Bitable app token" }),
  page_size: Type.Optional(
    Type.Number({ description: "Dashboards per page", minimum: 1, maximum: 100 }),
  ),
  page_token: Type.Optional(Type.String({ description: "Pagination token" })),
});
