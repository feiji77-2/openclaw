...
export const FeishuSheetsSchema = Type.Object({
  ...
  range: Type.Optional(
    Type.String({
      description:
        "[read_range/write_range/append] 范围。格式: SheetId!A1:C10 或 SheetId!A:C。SheetId 可通过 list_sheets 获取",
    }),
  ),
  // --- write_range / append ---
  values: Type.Optional(
    Type.Array(Type.Array(Type.Any(), { description: "行数据" }), {
  ...
  start_index: Type.Optional(
    Type.Number({ description: "[insert_rows] 插入位置（行索引，从 0 开始）" }),
  ),
