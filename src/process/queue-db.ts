...
type SqliteStatement = {
  run: (...args: unknown[]) => { lastInsertRowid?: number | bigint | string; changes: number };
...
export function initQueueDB(customDbPath?: string): SqliteDatabase {
  if (db) {
    return db;
  }
  ...
  `);
  // Add result column for legacy tables (if not exists)
  try {
  ...
  // pending_followup table: preserves queued messages in the followup queue to prevent loss on process crash
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_pending_followup_key ON pending_followup(queue_key);
  `);
...
// Record the time when this module is loaded (i.e. gateway startup time)
const moduleInitTime = Date.now();

// Atomically claim a task during dequeue to prevent concurrent runners from executing the same task
export function claimNextPendingTask(lane: string): TaskRecord | null {
  ...
  const transaction = conn.transaction(() => {
    // Find the oldest PENDING task in this lane
    const row = conn
      .prepare(`
    ...
    if (row) {
      ...
      // Inject __isRecoveredTask flag if this task is from a previous process execution
      let finalPayload = row.payload;
      if (row.created_at < moduleInitTime) {
        try {
          const parsed = JSON.parse(finalPayload);
          parsed.__isRecoveredTask = true;
      ...
      claimedTask = { ...row, payload: finalPayload, status: "RUNNING", updated_at: now };
    }
  });
...
// Check if there are any unfinished tasks (for graceful shutdown)
...
// Get all lanes that have PENDING tasks (used for recovery on startup)
...
// Mark stale PENDING/RUNNING tasks as FAILED (called on process restart)
...
// Get recoverable PENDING tasks (with full payload, used for channel recovery)
...
// ────────────────────────────────────────────────────────────────────────────
// pending_followup operations: preserve queued messages in the followup queue
// ────────────────────────────────────────────────────────────────────────────

/** Persist a followup queued item to SQLite */
export function insertPendingFollowup(queueKey: string, payload: any): number {
  const conn = initQueueDB();
  const now = Date.now();
  const info = conn
    .prepare("INSERT INTO pending_followup (queue_key, payload, created_at) VALUES (?, ?, ?)")
    .run(queueKey, JSON.stringify(payload), now);
  return info.lastInsertRowid as number;
}

/** Query all pending followup items (used during startup recovery) */
export function getAllPendingFollowups(): Array<{
  id: number;
  queue_key: string;
  payload: string;
  created_at: number;
  ...
  const conn = initQueueDB();
  return conn
    .prepare("SELECT id, queue_key, payload, created_at FROM pending_followup ORDER BY id ASC")
    .all() as Array<{ id: number; queue_key: string; payload: string; created_at: number }>;
}

/** Delete all pending followup items for a given queue_key (called after drain completes) */
export function clearPendingFollowupsByKey(queueKey: string): number {
  const conn = initQueueDB();
  const info = conn.prepare("DELETE FROM pending_followup WHERE queue_key = ?").run(queueKey);
  return info.changes;
}

/** Delete all pending followup items (called after full recovery is complete) */
export function clearAllPendingFollowups(): number {
  const conn = initQueueDB();
  const info = conn.prepare("DELETE FROM pending_followup").run();
  return info.changes;
}
